...
def plot_fluxes_helicity(post, args):
    ...
    for index, nc_path in enumerate(nc_paths):
        ...
        if args.time:
            ...
            fig_name_time = (
                f"helicity_fluxes_vs_{dimension_name}_time_{sim_label}"
            )
            ...
            ax_time.set_ylabel(r"$\Pi_H/\epsilon_W$")
    ...
    ax.set_ylabel(r"$\Pi_H/\epsilon_W$")
    ax.set_xscale("log")
    ax.set_box_aspect(0.5)
    ax.legend()

    # Save figure if required
    fig_name = f"helicity_fluxes_vs_{dimension_name}"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Plot the cumulative helicity transfers for the isothermal "
            "KREHM system."
        ),
    )
    ...
    plot_fluxes_helicity(post, args)
